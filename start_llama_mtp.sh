#!/bin/bash
# ============================================================
# Qwen3.8-27B llama-server 启动脚本 (MTP 看图版 —— 当前线上配置)
# 10.1.127.117  RTX 3090 24GB 专用
# ============================================================
# 功能:
#   - 纯文本 + 看图 (vision, mmproj Q8_0)
#   - 200K 上下文 (KV 混合精度: K=q8_0 V=q4_0)
#   - MTP 投机解码 n_max=4 (MTP 头内嵌主模型 GGUF, 无需 --model-draft)
...
#
# 日志:   ~/llama_mtp.log  (实时看: tail -f 该文件)
# 停止:   pkill -x llama-server
#
# 说明:
#   - 与 DFlash2 版 (start_llama_dflash2.sh) 仅差 spec-type / model-draft 两处
#   - MTP 图文通吃免补丁; DFlash2 需 speculative.cpp 补丁 (见部署文档 §7.1)
...
LOG=$HOME/llama_mtp.log
PIDF=$HOME/llama_mtp.pid
...
# ---- 启动 ----
echo ">>> [2/4] 启动 llama-server (MTP 看图版)"
echo "    附加参数: $*"
nohup "$BIN" \
  --model "$MODEL" \
  --mmproj "$MMPROJ" \
